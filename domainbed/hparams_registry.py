...
def _hparams(algorithm, dataset, random_seed):
    ...
    elif algorithm in ["FedSVD", "FedSVD2"]:
        _hparam('perturb_type', 'weight', lambda r: r.choice(['singular', 'weight']))
        _hparam('perturb_dist', 'uniform', lambda r: r.choice(['uniform', 'normal']))   # normal for weight
        _hparam('local_smooth', 0.1, lambda r: r.choice([1, 0.1, 0.01, 0]))
        _hparam('global_smooth', 0.1, lambda r: r.choice([1, 0.1, 0.01, 0.001, 0]))
        _hparam('perturb_init_scale', 0.1, lambda r: r.choice([1, 0.1, 0.01]))  # 0.1 for weight, 1 for singular
        _hparam('perturb_grad_scale', 0.01, lambda r: r.choice([1, 0.1, 0.01])) # 0.01 for weight, 1 for singular
        if algorithm == "FedSVD2":
            _hparam('perturb_layers', 'classifier', lambda r: r.choice(['layer4, classifier', 'classifier']))
